...
// Dynamically import Chart.js component with improved loading state
const DynamicChart = dynamic(
  () => import('./components/DynamicChart'),
  {
    ssr: false,
    loading: () => (
      <div className="h-72 flex items-center justify-center bg-slate-800/30 rounded-lg">
        <div className="animate-pulse text-blue-400">Loading Chart...</div>
      </div>
    ),
  }
);

// Firebase configuration matching Arduino setup
...
export default function Home() {
  ...
  const [waterHeightMeters, setWaterHeightMeters] = useState(0);
  ...
  // Fetch real-time water level data from Firebase
  useEffect(() => {
    ...
    const unsubscribe = onValue(waterLevelRef, (snapshot) => {
      ...
      if (data) {
        // Convert height in meters to percentage (0-3m mapped to 0-100%)
        const heightMeters = data.height || 0;
        const heightPercentage = Math.min(Math.round((heightMeters / 3) * 100), 100);
        
        setWaterHeightMeters(heightMeters.toFixed(2));
        setWaterLevel(heightPercentage);
        setRateOfChange(data.rate || 0);
        
        // Update timestamp
        const now = new Date();
        setLastUpdate(now.toLocaleString('id-ID', { 
          day: 'numeric', 
          month: 'short', 
          year: 'numeric',
          hour: '2-digit',
        ...
        // Add to historical data for chart (limited to last 6 data points)
  ...
  const chartData = {
    ...
    values: historicalData.map(item => item.value)
  };
      ...
      <Suspense fallback={<div>Loading...</div>}>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => {
                  const waterLevelRef = ref(database, '/water_level');
                  get(waterLevelRef);
                }} className="relative overflow-hidden px-4 py-2 rounded-full bg-gradient-to-r from-blue-500 to-blue-600 text-white text-sm font-medium transition-all hover:shadow-lg hover:from-blue-600 hover:to-blue-700 active:scale-95">
        ...
        <main className="relative z-10 max-w-7xl mx-auto px-6 py-12 space-y-12">
          ...
          <div className={fadeIn}>	
              ...
              <div className="space-y-8">
                      ...
                      <div className="flex flex-col items-center">
                        <div className="relative w-48 h-48 flex items-center justify-center">
                          ...
                          <div className="absolute flex flex-col items-center">
                            ...
                            <div className="text-xs text-gray-400 mt-2">{waterHeightMeters} meter</div>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="lg:col-span-3 card-hover">
                    <div className="bg-slate-800/50 backdrop-blur-md rounded-2xl p-8 border border-slate-700">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="col-span-1 bg-gray-800/30 backdrop-blur-sm border border-white/5 rounded-lg p-3">
                          <p className="text-xs text-gray-400 mb-1">Kecepatan Perubahan</p>
                          <div className="flex items-center">
                            <svg className="w-5 h-5 mr-2 text-purple-300" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" />
                            </svg>
                            <p className="font-medium">{rateOfChange.toFixed(3)} m/s</p>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="bg-slate-800/50 backdrop-blur-md rounded-2xl p-8 border border-slate-700 card-hover">
                  <h3 className="text-lg font-medium text-gray-100 mb-4">
                    Riwayat Ketinggian Air (Pemantauan Real-time)
                  </h3>
                  <div className="h-72">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-8">
                  <div className="bg-slate-800/50 backdrop-blur-md rounded-2xl p-6 border border-slate-700 card-hover">
                    <div className="flex items-center space-x-2">
                      <IoWaterOutline className="text-blue-400 text-xl" />
                      <h3 className="text-blue-100 text-sm">Status Ketinggian</h3>
                    </div>
                    <p className="text-2xl font-semibold mt-2 text-white">
                      {waterLevel > 80 ? 'Bahaya' : waterLevel > 60 ? 'Waspada' : 'Normal'}
                    </p>
                    <span className={`${waterLevel > 80 ? 'text-red-400' : waterLevel > 60 ? 'text-yellow-400' : 'text-green-400'} text-sm`}>
                      {waterLevel}% dari maksimum
                    </span>
                  </div>
                  ...
                  <div className="bg-slate-800/50 backdrop-blur-md rounded-2xl p-6 border border-slate-700 card-hover">
                    <div className="flex items-center space-x-2">
                      ...
                      <h3 className="text-blue-100 text-sm">Arah Perubahan</h3>
                    </div>
                    <p className="text-2xl font-semibold mt-2 text-white">
                      {rateOfChange > 0.01 ? 'Naik' : rateOfChange < -0.01 ? 'Turun' : 'Stabil'}
                    </p>
                    <span className={`${rateOfChange > 0.01 ? 'text-red-400' : rateOfChange < -0.01 ? 'text-green-400' : 'text-blue-400'} text-sm`}>
                      {Math.abs(rateOfChange).toFixed(3)} m/s
                    </span>
                  </div>
                </div>
              </div>
            ...
            {activeTab === 'analisis' && (
              <div className="bg-slate-800/50 backdrop-blur-md rounded-2xl p-8 border border-slate-700">
                <h2 className="text-xl font-semibold mb-4 text-blue-100">Analisis Data</h2>
                <p className="text-gray-300 mb-6">Halaman ini akan menampilkan analisis prediktif ketinggian air.</p>
                <div className="p-8 text-center">
                  <p className="text-gray-400">Fitur analisis sedang dalam pengembangan</p>
                </div>
              </div>
            )}
            ...
            {activeTab === 'pengaturan' && (
              <div className="bg-slate-800/50 backdrop-blur-md rounded-2xl p-8 border border-slate-700">
                <h2 className="text-xl font-semibold mb-4 text-blue-100">Pengaturan</h2>
                <p className="text-gray-300 mb-6">Konfigurasi sistem pemantauan dan notifikasi.</p>
                <div className="p-8 text-center">
                  <p className="text-gray-400">Fitur pengaturan sedang dalam pengembangan</p>
                </div>
              </div>
            )}
          </div>
